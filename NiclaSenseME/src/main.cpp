...
/* Includes ---------------------------------------------------------------- */
#include <Arduino.h>
#include <nicla3_inferencing.h>
#include "Arduino_BHY2.h" //Click here to get the library: http://librarymanager/All#Arduino_BHY2
#include <ArduinoBLE.h>
#include "Nicla_System.h"

#define BLE_UUID_EI_SERVICE "9A48ECBA-2E92-082F-C079-9E75AAE428B1"
#define BLE_UUID_PREDICTION_CHARACTERISTIC    "2D2F88C4-F244-5A80-21F1-EE0224E80658"

BLEService edgeImpulseService(BLE_UUID_EI_SERVICE);
BLEStringCharacteristic predictionChar(BLE_UUID_PREDICTION_CHARACTERISTIC, BLERead | BLENotify, 64);
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
          ...
          sendData += String(result.classification[ix].label) + "," + String(result.classification[ix].value,5) + ",";
          //Serial.println(sendData);
      }
      predictionChar.writeValue(sendData);
